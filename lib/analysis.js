...
### WHAT TO OBSERVE IN THE VIDEO FRAMES

You are given a sequence of frames sampled evenly across a drill video, in chronological order. You do NOT have joint-angle or biomechanical tracking data — evaluate the drill directly from what is visible in the images, watching them as a sequence. Translate the Alive Movement Problem Checklist into what you can actually see:

- **Opponents / defenders**: Is there a real opponent visible in the frames? Are they moving and reacting like a live opponent, or standing/moving passively and unrealistically?
- **Real goal**: Is there a visible goal channeling the movement (a ball, a gap to win, a defender to beat, a route run against real coverage) or does the activity look like goal-less repetition?
- **Decision-making vs. predetermined pattern**: Do the athletes appear to be reading and reacting to something in front of them, or executing a memorized, identical pattern regardless of what's around them?
- **Unpredictability**: Does anything meaningful change between reps across the frames, or does the picture look identical each time (same spacing, same path, same outcome)?
- **Lines and passive waiting**: Are players standing in a line waiting for a turn? Are teammates or "defenders" just standing around rather than actively participating?
- **Traditional equipment as movement signifiers**: Are cones, bags, ladders, chutes, or dummies visible and being used to dictate where/when/how to move, in place of real opposition or real information?
- **Information sources athletes are coupling their movement to**: What is actually specifying the athletes' movement in these frames — a cone/bag/ladder (dead, pre-planned), or a moving opponent's body position, a ball, a teammate's positioning, real field spacing (alive, information-driven)?

Base your Aliveness Score, Dead Drill Assessment, and Constraint Manipulation suggestions on what is visibly observable across these frames. If the image quality, camera angle, or frame count don't give you enough information to assess something confidently, say so plainly rather than guessing.
...
// Parses a "data:image/jpeg;base64,...." data URL into the parts the Anthropic API needs.
...
// Drill Analysis is video-frame-based, not skeleton-based: the frontend samples several evenly
// spaced JPEG frames from the uploaded clip and sends them here as image content blocks alongside
// the coach's description, so Claude can evaluate the drill directly from what it can see — this
// works on wide-angle, multi-person practice footage where pose estimation breaks down.
function buildDrillAnalysisContent({ duration, context, frames }) {
  const introText = [
    '## DRILL ANALYSIS REQUEST (VIDEO FRAMES)',
    '',
    `Video duration: ${duration}`,
    `Number of sampled frames: ${frames.length} (evenly spaced across the clip, in chronological order)`,
    context ? `Drill description: ${context}` : "No drill description provided — infer the drill's intent from the frames and flag that context is limited.",
    '',
    'The images below are frames sampled evenly across the drill video, in chronological order.',
    'Watch them as a sequence to understand what is happening in the drill.',
    '',
  ...
  const outroText = [
    '---',
    '',
    'Evaluate this drill against the Alive Movement Problem Checklist from the Yearby/Ecological',
    'Dynamics framework, using what you can observe across these frames (see "WHAT TO OBSERVE IN',
    'THE VIDEO FRAMES" in the system prompt).',
    '',
...
// Injury Prevention is unchanged from prior sessions: MediaPipe joint-angle data, text-only prompt.
async function runInjuryAnalysis({ duration, totalFrames, context, angles }) {
  ...
  const userMessage = buildInjuryPreventionMessage({ duration, totalFrames, context, angles });
  ...
  return { analysis: text, mode: 'injury' };
}

// Drill Analysis is video-frame-based (multimodal): sampled JPEG frames + coach context, no pose
// data required at all — this is what makes it work on wide-angle, multi-person practice footage.
async function runDrillAnalysis({ duration, context, frames }) {
  if (!Array.isArray(frames) || frames.length < 1) {
    const err = new Error('No video frames received for analysis.');
    err.status = 400;
    throw err;
  }

  const content = buildDrillAnalysisContent({ duration: duration || 'unknown', context, frames });
  ...
  const text = msg.content.find(b => b.type === 'text')?.text ?? '';
  return { analysis: text, mode: 'drill' };
}
...
export async function runAnalysis(payload) {
  ...
  return payload.mode === 'injury'
    ? runInjuryAnalysis(payload)
    : runDrillAnalysis(payload);
}
